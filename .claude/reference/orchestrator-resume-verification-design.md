...
- `INTEXURAOS_GEMINI_APP_API_KEY` (required secret)

Hard gate policy:

- completion verifier is always enabled
- completion verifier is Gemini-only
- verifier model is fixed to `gemini-2.5-flash`
- orchestrator startup fails if `INTEXURAOS_GEMINI_APP_API_KEY` is missing
...
If SLO is not met, tune prompt/extraction policy first; provider/model changes require explicit code+config change.
