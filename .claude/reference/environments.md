...
The retained transcription Cloud Function receives its runtime `INTEXURAOS_SENTRY_DSN` value from the dedicated Secret Manager secret `INTEXURAOS_SENTRY_DSN_DEV`. That secret is excluded from shared Cloud Run IAM and from the Hetzner provisioner/runtime secret inventories. During the staged cutover, the transcription service account temporarily keeps secret-specific `roles/secretmanager.secretAccessor` bindings for both `INTEXURAOS_SENTRY_DSN` and `INTEXURAOS_SENTRY_DSN_DEV`; this prevents Terraform from removing access needed by the old function revision before the new revision is active. After the `_DEV` revision is verified, the follow-up cleanup removes only the legacy binding. No other workload runtime identity receives a secret-specific binding for `_DEV`; the administrative Cloud Build service account retains its project-wide access for function deployment.
