...
**Environments:** local=`localhost` (same data/async pattern as dev, PM2 watch + Vite from current checkout) | dev=`dev.intexuraos.cloud` (same data/async pattern, PM2 on home-dev) | prod=`intexuraos.cloud` (Hetzner PM2/nginx + retained GCP data plane). All environments use the SAME retained GCP project `intexuraos-dev-pbuchman` (the `-dev-pbuchman` suffix is legacy — there is no separate prod project). Local/dev/prod share Firestore/Auth0/Secret Manager; local and dev each use their own Pub/Sub emulator. The practical local-vs-dev difference is where the process tree runs. Reference: `.claude/reference/environments.md`
