...
**Code Task Callback Ownership:** Orchestrator is deployment-independent. `workerLocation` is only the machine executing the task, never the task owner. The task's required HTTP(S) `webhookUrl` determines callback ownership. Every new callback is production-owned under `https://intexuraos.cloud/api/code/internal/...`; a DEV URL is historical/recovery input, not a fallback. Reference: `.claude/reference/environments.md`, `.claude/reference/architecture.md`
...
**Environments:** local=`localhost` (PM2 watch + Vite from the current checkout) | DEV is a retained configuration label and is normally hibernated (`dev.intexuraos.cloud` returns `503`) | prod=`intexuraos.cloud` (Hetzner PM2/nginx + retained GCP data plane). Home Dev is a production-owned worker host; it keeps the orchestrator and unrelated shared services but not the DEV PM2/emulator runtime. All labels use the same retained GCP project `intexuraos-dev-pbuchman`; its suffix is legacy. Reference: `.claude/reference/environments.md`

**Code Task Investigation:** When user pastes `dev.intexuraos.cloud/#/code-tasks/task_*` or `intexuraos.cloud/#/code-tasks/task_*` URL — use `/debug-code-task` skill. Treat the DEV hostname only as historical link provenance. NEVER WebFetch/curl the SPA URL (hash routing returns shell HTML). Data is in Firestore `code_tasks` collection.
...
**Investigation Discipline:** When a tool, binary, or service crashes: (1) investigate the environment (arch, resources, config), not the tool. (2) Trace the causal chain to fixable code — surface symptoms are not root causes; keep asking "why?". (3) Diagnosis without fix is incomplete — implement and verify the fix. (4) On Home Dev, read `/var/lib/intexuraos-dev/runtime-mode.env` first and never start DEV-owned PM2/emulator units outside the reviewed `intexuraos-dev-mode` resume workflow. (5) Confident speculation is a violation — "crashes because X" without evidence is as wrong as "maybe X". Reference: `.claude/reference/investigation-discipline.md`
