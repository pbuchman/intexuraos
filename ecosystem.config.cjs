...
const COMMON_SERVICE_ENV = {
  ...
  ...(process.env.PREDEV_ENVIRONMENT !== 'true' && { FIREBASE_AUTH_EMULATOR_HOST: 'localhost:8104' }),
};
...
const SERVICE_ENV_MAPPINGS = {
  ...
  'code-agent': {
    ...
    INTEXURAOS_ORCHESTRATOR_SECRET:
      process.env.INTEXURAOS_ORCHESTRATOR_SECRET ?? 'dev-orchestrator-secret',
    INTEXURAOS_PUBSUB_WHATSAPP_SEND_TOPIC:
      process.env.INTEXURAOS_PUBSUB_WHATSAPP_SEND_TOPIC ?? 'whatsapp-send-message',
    INTEXURAOS_TOKEN_ENCRYPTION_KEY: process.env.INTEXURAOS_TOKEN_ENCRYPTION_KEY ?? '',
    INTEXURAOS_GITHUB_WEBHOOK_SECRET:
      process.env.INTEXURAOS_GITHUB_WEBHOOK_SECRET ?? 'dev-github-webhook-secret',
  },
  'bookmarks-agent': {
  ...
  apps: [
    ...
    // because Cloud Functions gateway doesn't support WebSocket
    {
      ...
      env: {
        ...
        VITE_PM2_MODE: 'true',
        ...(process.env.PREDEV_ENVIRONMENT !== 'true' && { INTEXURAOS_USE_FIREBASE_EMULATORS: 'true' }),
      },
      autorestart: true,
      max_restarts: 5,
      restart_delay: 2000,
