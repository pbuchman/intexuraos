...
export function CodeTaskLogsModal({ taskId, onClose }: CodeTaskLogsModalProps): React.JSX.Element {
  ...
  return (
    <Modal
      ...
      title="Task Logs"
      description={task !== null ? `Task ${task.id}` : `Task ${taskId}`}
      hideTitle
      padded={false}
      contentClassName="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 flex max-h-[90vh] w-full max-w-5xl flex-col rounded-xl bg-white shadow-2xl dark:bg-slate-900"
    >
