...
const codeTaskSchema = {
  ...
  properties: {
    ...
    workerType: { type: 'string', enum: ['opus', 'auto', 'sonnet', 'minimax', 'glm', 'qwen'] },
...
export const codeRoutes: FastifyPluginCallback<CodeRoutesOptions> = (fastify, opts, done) => {
  ...
  fastify.post<{
    ...
    async (request: FastifyRequest<{ Body: { actionId: string; approvalEventId: string; userId: string; payload: { prompt: string; workerType?: 'opus' | 'auto' | 'sonnet' | 'minimax' | 'glm' | 'qwen'; linearIssueId?: string; repository?: string; baseBranch?: string } } }>, reply: FastifyReply) => {
...
/* v8 ignore start -- ts-type: TypeScript type narrowing makes branch unreachable @preserve */
  ...
  fastify.post<{
    ...
    async (request: FastifyRequest<{ Body: { prompt: string; workerType?: 'opus' | 'auto' | 'sonnet' | 'minimax' | 'glm' | 'qwen'; workerLocation?: string; linearIssueId?: string } }>, reply: FastifyReply) => {
      logIncomingRequest(request, {
  ...
  fastify.post(
      ...
      schema: {
        ...
        body: {
          ...
          properties: {
            ...
            workerType: {
              type: 'string',
              enum: ['opus', 'auto', 'sonnet', 'minimax', 'glm', 'qwen'],
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (workerType !== undefined && ['opus', 'auto', 'sonnet', 'minimax', 'glm', 'qwen'].includes(workerType)) {
        retryRequest.workerType = workerType as 'opus' | 'auto' | 'sonnet' | 'minimax' | 'glm' | 'qwen';
      }
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Only add workerType if provided and valid
      const executionAgentRequest: { originalTaskId: string; userId: string; workerType?: 'opus' | 'auto' | 'sonnet' | 'minimax' | 'glm' | 'qwen' } = { originalTaskId: taskId, userId };
      if (requestedWorkerType !== undefined && ['opus', 'auto', 'sonnet', 'minimax', 'glm', 'qwen'].includes(requestedWorkerType)) {
        executionAgentRequest.workerType = requestedWorkerType as 'opus' | 'auto' | 'sonnet' | 'minimax' | 'glm' | 'qwen';
      }
