...
import type { IntexAgentClient } from '../clients/intexAgentClient.js';
import type { TaskStatus } from '../../domain/models/codeTask.js';

/**
 * Status of the associated resource (e.g., code task).
 * Must match intex-agent's ResourceStatus type exactly.
 * See: apps/intex-agent/src/domain/models/action.ts
 */
 ...
 * Real intex-agent IDs are standard UUIDs (crypto.randomUUID()).
...
export function createStatusMirrorService(deps: StatusMirrorServiceDeps): StatusMirrorService {
  const { intexAgentClient, logger } = deps;

  // Map TaskStatus → ResourceStatus (intex-agent vocabulary)
  ...
  return {
    async mirrorStatus(params): Promise<void> {
      ...
      // These are generated by non-intex-agent flows for dedup purposes only.
      // See: INT-1119
      ...
      const result = await intexAgentClient.updateActionStatus(
        actionId,
        resourceStatus,
