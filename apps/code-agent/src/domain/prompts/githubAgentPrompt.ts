...
function buildPRSection(input: GitHubAgentPromptInput): string[] {
  ...
  return [
    ...
    '- **security**: Security-focused review. Request when changes touch auth, authorization, secrets, tokens, user input, API endpoints, external input parsing, command execution, or webhook validation.',
    '- **architecture**: Design review. Request when changes are cross-service, span multiple packages/services, introduce new patterns, alter dependency direction, change data ownership, or affect scalability/coupling.',
    '- **test_quality**: Test review. Request when PR has significant test file changes (.test.ts, .spec.ts) or test infrastructure changes. Checks for false positives, isolation, assertion strength, v8 ignore legitimacy, and test design.',
    '- **documentation**: Documentation review. Request for docs-only non-plan PRs and for substantial docs changes mixed with code. Checks accuracy against code, broken or obsolete instructions, missing prerequisites, unclear examples, and internal consistency.',
    '',
    ...
    '1. Call `skip({"reason":"Generated metadata only; no review needed."})`',
    '2. Respond: "Skipped generated metadata-only PR."',
  ];
}
