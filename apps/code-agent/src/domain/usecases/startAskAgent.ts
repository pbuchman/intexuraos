...
export async function startAskAgent(
  ...
  if (!createResult.ok) {
    ...
    /* v8 ignore start -- test-infra: v8 coverage cannot capture branch hit through FakeFirestore in CI forks pool environment; test passes but coverage reports uncovered @preserve */
