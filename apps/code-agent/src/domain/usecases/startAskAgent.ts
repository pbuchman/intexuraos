...
export async function startAskAgent(
  ...
  if (!createResult.ok) {
    ...
    /* v8 ignore next -- test-infra: FakeFirestore cannot simulate write failures, so codeTaskRepo.create() cannot return a non-DUPLICATE_PROMPT error code @preserve */
