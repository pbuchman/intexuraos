...
describe('WorkerHealthProbe', () => {
  ...
  describe('probeWorker', () => {
    ...
    it('preserves every supported provider API-key validation status', async () => {
      const providerApiKeys = {
        MISSING_KEY: { configured: false, status: 'missing' },
        PENDING_KEY: { configured: true, status: 'unknown' },
        VALID_KEY: { configured: true, status: 'valid' },
        INVALID_KEY: { configured: true, status: 'invalid' },
        DEGRADED_KEY: { configured: true, status: 'degraded' },
      };
      mockedFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => readyHealth({ providerApiKeys }),
      } as Response);

      const result = await probe.probeWorker(mockWorker);

      expect(result).toMatchObject({ _tag: 'healthy', providerApiKeys });
    });

    it('rejects unknown provider API-key validation statuses', async () => {
      mockedFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => readyHealth({
          providerApiKeys: {
            KIMI_API_KEY: { configured: true, status: 'surprising' },
          },
        }),
      } as Response);
    ...
    it.each([
      ...
      { KIMI_API_KEY: null },
      { KIMI_API_KEY: {} },
      { KIMI_API_KEY: { configured: 'yes' } },
    ])('rejects malformed provider API-key health entries', async (providerApiKeys) => {
      mockedFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => readyHealth({ providerApiKeys }),
