...
describe('github-event-parser', () => {
  ...
  describe('parseGitHubWebhookEvent', () => {
    ...
    it('should parse check_suite events via parseGitHubWebhookEvent', () => {
      const payload = {
        action: 'completed',
        check_suite: {
          id: 123,
  ...
  describe('parseCheckSuiteEvent', () => {
    ...
    it('should pass through Date object for merged_at', () => {
      const mergedDate = new Date('2024-01-15T12:00:00.000Z');
      const payloadWithMergedAtDate = {
        action: 'completed',
        check_suite: {
          id: 123,
          head_branch: 'task_abc123',
          ...
          pull_requests: [
            {
              ...
              body: 'Description',
              state: 'open',
              merged_at: mergedDate, // Pass Date object directly
            },
          ],
        },
        repository: {
      ...
      const result = parseCheckSuiteEvent(payloadWithMergedAtDate);

      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).not.toBeNull();
        if (result.value !== null) {
          expect(result.value.mergedAt).toBe(mergedDate); // @allow-result-access -- Date instance passed through
        }
      }
    });

    it('should return error when sender is missing', () => {
      const payloadWithMissingSender = {
        action: 'completed',
        check_suite: {
          id: 123,
          head_branch: 'task_abc123',
      ...
      const result = parseCheckSuiteEvent(payloadWithMissingSender);

      expect(result.ok).toBe(false); // @allow-result-access -- testing error path
    });

    it('should return error when repository is missing', () => {
      const payloadWithMissingRepo = {
        action: 'completed',
        check_suite: {
          id: 123,
          head_branch: 'task_abc123',
      ...
      const result = parseCheckSuiteEvent(payloadWithMissingRepo);

      expect(result.ok).toBe(false); // @allow-result-access -- testing error path
    });

    it('should return error when repository has invalid data', () => {
      const payloadWithInvalidRepo = {
        action: 'completed',
        check_suite: {
          id: 123,
          head_branch: 'task_abc123',
      ...
      const result = parseCheckSuiteEvent(payloadWithInvalidRepo);

      expect(result.ok).toBe(false); // @allow-result-access -- testing error path
    });

    it('should return error when PR data has invalid types', () => {
      const payloadWithInvalidPR = {
        action: 'completed',
        check_suite: {
          id: 123,
          head_branch: 'task_abc123',
      ...
      const result = parseCheckSuiteEvent(payloadWithInvalidPR);

      expect(result.ok).toBe(false); // @allow-result-access -- testing error path
    });

    it('should handle non-string head_sha', () => {
      const payloadWithNonStringHeadSha = {
        action: 'completed',
        check_suite: {
          id: 123,
          head_branch: 'task_abc123',
          head_sha: 12345, // Should be string
          conclusion: 'failure',
          pull_requests: [
            {
              id: 101,
      ...
      const result = parseCheckSuiteEvent(payloadWithNonStringHeadSha);
    ...
    it('should use default checkSuiteId when not a number', () => {
      const payloadWithNonNumericCheckSuiteId = {
        action: 'completed',
        check_suite: {
          id: 'not-a-number', // Should be number
          head_branch: 'task_abc123',
      ...
      const result = parseCheckSuiteEvent(payloadWithNonNumericCheckSuiteId);
