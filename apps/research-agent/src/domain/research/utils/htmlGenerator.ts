...
export interface GeneratedByUserInfo {
  name?: string;
  email?: string;
}
...
function renderGeneratedByInfo(
  ...
  const { name, email } = generatedBy;

  if (name !== undefined && name !== '') {
    const emailPart =
      email !== undefined && email !== '' ? ` (${escapeHtml(email)})` : '';
    return `<p class="meta">Generated by ${escapeHtml(name)}${emailPart} with IntexuraOS on ${formattedDate}</p>`;
  }

  if (email !== undefined && email !== '') {
    return `<p class="meta">Generated by ${escapeHtml(email)} with IntexuraOS on ${formattedDate}</p>`;
  }
