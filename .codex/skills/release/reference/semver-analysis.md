...
1. Use the available Linear connector to query the exact issue ID with limit 10. Do not run broad text searches.
2. Extract title, description, labels, state, and parent issue ID.
3. Use the Linear connector to list child issues with `parentId` and limit 20.
4. For each child issue, extract title, description, labels, and state.
5. If Linear connector access is unavailable, stop and report that Linear issue details cannot be collected; do not guess from PR titles alone.
...
If the manifest is empty (no PRs, no direct commits), ask the user how to proceed:
...
**This step is executed as part of the Phase 1 single prioritization touchpoint in `full-release.md` step 1.7.** The categorized changes from Step 5 are presented to the user in a consolidated view. Do not create separate user prompts here.
...
2. **Fetch issue details:** Use the available Linear connector to query the exact issue ID with limit 10.
...
4. **Fetch subissues:** Use the Linear connector with the parent issue ID and limit 20.
