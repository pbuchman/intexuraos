...
This document compares the current GCP-backed production environment with the Hetzner migration foundation introduced by INT-1633. It is based on the current PR diff, `terraform/shared-gcp/main.tf`, `terraform/hetzner-prod/**`, `apps/web/service-manifest.json`, `ecosystem.config.cjs`, `firestore-collections.json`, the INT-1632 parent plan, and the project environment rules.

## Current State

IntexuraOS currently has one GCP project, `intexuraos-dev-pbuchman`, and one GCP Terraform root, `terraform/shared-gcp/`. The `dev` name is historical: this root owns infrastructure for the public production domain `intexuraos.cloud` and retained shared resources used by both production and `dev.intexuraos.cloud`.
...
| --- | --- | --- |
| Directory | `terraform/shared-gcp` | `terraform/hetzner-prod` |
...
| Plan command | `STORAGE_EMULATOR_HOST= FIRESTORE_EMULATOR_HOST= PUBSUB_EMULATOR_HOST= GOOGLE_APPLICATION_CREDENTIALS=$HOME/.config/gcloud/sa-key.json terraform -chdir=terraform/shared-gcp plan` | `STORAGE_EMULATOR_HOST= FIRESTORE_EMULATOR_HOST= PUBSUB_EMULATOR_HOST= GOOGLE_APPLICATION_CREDENTIALS=$HOME/.config/gcloud/sa-key.json HCLOUD_TOKEN=$HCLOUD_TOKEN terraform -chdir=terraform/hetzner-prod plan` |
| Apply command | `STORAGE_EMULATOR_HOST= FIRESTORE_EMULATOR_HOST= PUBSUB_EMULATOR_HOST= GOOGLE_APPLICATION_CREDENTIALS=$HOME/.config/gcloud/sa-key.json terraform -chdir=terraform/shared-gcp apply` | `STORAGE_EMULATOR_HOST= FIRESTORE_EMULATOR_HOST= PUBSUB_EMULATOR_HOST= GOOGLE_APPLICATION_CREDENTIALS=$HOME/.config/gcloud/sa-key.json HCLOUD_TOKEN=$HCLOUD_TOKEN terraform -chdir=terraform/hetzner-prod apply` |
...
| --- | --- | --- | --- |
| Public compute | 22 Cloud Run app services in `terraform/shared-gcp/main.tf` | Target is one Hetzner VM running Node services under PM2 behind nginx | Cloud Run app traffic moves to nginx and local PM2 processes after later runtime and cutover work |
...
| Terraform state | `terraform/shared-gcp` owns GCP state | `terraform/hetzner-prod` owns Hetzner foundation state | State is provider-isolated; no `terraform/environments/prod` GCP root is created |
...
| Firestore | `(default)` database managed by `terraform/shared-gcp` | Retained; exposed as `retained_firestore_database_id` and inventory only | Stays in GCP and remains shared |
...
| Monitoring | Existing monitoring module remains in `terraform/shared-gcp` | No replacement in INT-1633 | Monitoring cleanup or replacement is out of scope for this foundation PR |
...
| Async routes | Add Pub/Sub/Scheduler Terraform in `terraform/shared-gcp` | Add Hetzner-targeted push or scheduler destination in the cutover Terraform owned by follow-up work |
...
| Who owns it? | `terraform/shared-gcp` through the Firestore module |
...
7. Remove obsolete GCP resources from `terraform/shared-gcp` in small reviewed changes, each with a plan proving no retained data resource is destroyed.
...
The April Hetzner plan proposed `terraform/environments/prod/`, but current project rules and INT-1632 supersede that: the active design uses `terraform/hetzner-prod/` for Hetzner and keeps `terraform/shared-gcp/` as the only GCP root. The stale plan also treated `api-docs-hub` as optional; current Terraform and PM2 config include it as a service, while the web service manifest still excludes it from frontend URL injection.
