...
| Pushed code/evidence baseline | `02018515f75eb02c03a8990861cd938142b96b18`, `c804f759193569b6f78ef4699a2607004f17938d`, `32e22ed5f3553fe556f5aed53e152a5362ead07a`, `27c0912ec89a7f1319180606d80886b2928cb738`, `59a709e61b83a9aee4a84206343eb33a05297d7d`; the current App Check/inventory revision is not yet committed |
| Current working state | Draft PR `#2454` is open and merge-clean against `development`; all 15 applicable GitHub checks pass on pushed commit `59a709e61b83a9aee4a84206343eb33a05297d7d`. The current App Check/audit/inventory revision is uncommitted. A complete pre-evidence-edit `pnpm run ci:tracked` candidate run passed all phases with `8010` tests at 2026-08-14 00:15 Europe/Warsaw; the exact commit remains gated on a second local run after this evidence-only edit and on the PR checks for the pushed commit. Terraform-managed retained-GCP resources are converged, but live project IAM still contains two unmanaged Cloud Build service-agent `roles/secretmanager.admin` bindings pending Terraform adoption and least-privilege cleanup. Credential-gated DEV/PROD rollout and the Firebase, runtime-SA, and legacy-read observation intervals remain PENDING; no soak `T0` has begun. |
...
- Commits through `59a709e61b83a9aee4a84206343eb33a05297d7d` are pushed to
  `origin/codex/secret-packages-production`; the current App Check/audit/inventory revision is
  uncommitted. Draft PR `#2454` targets `development`. Nothing has been merged and no production
  deployment has run.
...
- A fresh, verified provider credential set must be used to publish byte-identical replacement DEV
...
- The pushed hardening adds durable publication recovery, crash-atomic DEV projection,
  ...
  interrupted stable-link activation. A complete pre-evidence-edit local `ci:tracked` candidate run
  passes all phases with `8010` tests; the exact commit is accepted only after the required second
  local run and its pushed PR checks are green. The required GCP topic plus package-scoped publisher
  metadata IAM are applied with a fresh `No changes` plan.
...
- [ ] Replace the three credentials rejected by live upstream probes, resolve the MiniMax quota/rate
  failure, and build current DEV candidates `v3` and `v4` from exact sources; require them to be
  byte-identical before rollout.
...
| Runtime/Hetzner/orchestrator tests | PASS | 2026-08-14 00:15 Europe/Warsaw: included in the complete pre-evidence-edit `ci:tracked` candidate run, which passed `8010` tests |
...
| Documentation format/diff checks | PASS | 2026-08-14 00:15 Europe/Warsaw: repository-wide format phase and post-build checks passed in the complete pre-evidence-edit `ci:tracked` candidate run; `git diff --check` PASS |
| `pnpm run typecheck:tests` | PASS | 2026-08-13 20:56 Europe/Warsaw: current-tree test typecheck PASS |
| `pnpm run ci:tracked` | PASS, complete exact-commit run | 2026-08-14 00:15 Europe/Warsaw: pre-evidence-edit candidate run exited `0`; Type/Lint, Static Validation, `8010` tests with coverage, Coverage Validation, Web Build & Format, and Post-Build Checks all passed. This row deliberately requires a second full local run after its own evidence edit; the pushed commit's PR checks are the non-self-referential exact-commit proof. |
...
| 2026-08-13 23:35 Europe/Warsaw | Cloud Build service-agent least privilege | IAM v3 captured 2026-08-13T21:32:19Z (etag `BwZY8iQeXcI=`) showed two orphan project `roles/secretmanager.admin` bindings for `service-544224260556@gcp-sa-cloudbuild.iam.gserviceaccount.com`. The unconditional binding was added 2026-04-07T22:29:02.207978Z by `claude-code-dev` using `gcloud`; the conditional binding was added 2026-01-21T00:30:54.634865Z by `kontakt@pbuchman.com` through Chrome/Console with title `cloudbuild-connection-setup` and expiry 2026-01-21T01:30:54.054Z. Neither is in Terraform state. The connection is `COMPLETE`, non-disabled, non-reconciling; metadata-only `fetchGitRefs` at 21:33:55Z passed with 19 refs including `main` and `development`. Removal remains PENDING until reauthentication proves or creates the exact OAuth-token secret-level accessor; the runbook requires Terraform adoption, zero-change import plan, exact two-delete saved plan, post-apply canary, and narrow rollback. |
| 2026-08-14 00:15 Europe/Warsaw | Pre-evidence-edit local code verification | `pnpm run ci:tracked` exited `0`: Type/Lint PASS (`152.4s`), Static Validation PASS (`20.5s`), `8010/8010` tests with coverage PASS (`540.9s`) with no unexpected test stdout, Coverage Validation PASS (`1.2s`), Web Build & Format PASS (`20.9s`), and Post-Build Checks PASS (`0.1s`). Because recording this result changes the goal document, a second complete local run is mandatory before commit; PR checks on the resulting commit provide the exact-tree evidence. |
