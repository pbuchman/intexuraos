...
> [!NOTE]
> **Skill is NOT context-aware.** The difference between orchestrator and CLI execution is that the orchestrator's system prompt provides complete context (issue description, test requirements, etc.) *before* invoking `/linear`. The skill itself does NOT detect or branch on execution context.
