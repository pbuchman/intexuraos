...
This is a prompt-focused change, but the follow-up implementation must account for deterministic Intex agent replies that bypass the LLM. `apps/intex-agent/src/domain/agent/intexAgentRunner.ts` currently returns hard-coded Polish text for the greeting short-circuit, confirmation previews, completed tool replies, and unsupported-intent replies. `apps/intex-agent/src/domain/agent/intentGate.ts` owns the greeting classification that can trigger the no-action greeting path before the prompt is used. Those paths cannot be localized by prompt text alone, so the implementation scope must either localize those deterministic replies or explicitly narrow the acceptance target to LLM-generated replies and document the remaining Polish-only behavior.
