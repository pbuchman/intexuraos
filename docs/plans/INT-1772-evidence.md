...
- Implement deterministic reply behavior in `apps/intex-agent/src/domain/agent/intexAgentRunner.ts` by localizing hard-coded confirmation/completed/unsupported/greeting replies according to the same last-reasonable-user-message rule. Address `apps/intex-agent/src/domain/agent/intentGate.ts` as the greeting classifier and `intexAgentRunner.ts` as the file that returns the no-action greeting so trivial greetings do not force Polish.
...
- Maintain 100% branch coverage for new or changed prompt and deterministic-reply logic in `systemPrompt.ts` and `intexAgentRunner.ts`; cover any new branch with tests rather than coverage ignores.
