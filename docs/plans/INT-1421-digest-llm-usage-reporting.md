...
5. **Env var already available**: `INTEXURAOS_LLM_USAGE_SERVICE_URL` is declared in Terraform `common_service_env_vars` (`terraform/shared-gcp/main.tf:302`) and in PM2 `COMMON_SERVICE_URLS` (`ecosystem.config.cjs:59`). Both environments already inject it into `mobile-notifications-service`; the service just never reads it.
