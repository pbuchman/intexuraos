# Retired Scheduler Service Service — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Introduce a new `retired-scheduler-service` service that manages user-defined recurring schedules described in human language, executes them as agentic tool-calling workflows on schedule, logs all execution events, and provides a web UI for managing schedules and viewing execution logs.

**Architecture:** A new Fastify app (`apps/retired-scheduler-service`) owns two Firestore collections (`retired-scheduler-schedules`, `retired-scheduler-executions`). Cloud Scheduler calls `POST /internal/cron/tick` every minute. The tick handler evaluates all active schedules against current time, and for each due schedule, executes the configured action using an LLM tool-calling agent. Actions are NOT simple HTTP calls — they are step-based sequences (including conditionals) described in human language and executed by a Gemini tool-calling agent. Tools are dynamically generated from each allowlisted service's OpenAPI spec (`/openapi.json`), filtered to `/internal/*` endpoints. Each service's tools are namespaced (e.g., `code_agent__get_running_tasks`) to avoid collisions. At schedule creation time, an LLM also converts the human-language schedule description into a cron expression. The web app (`apps/web`) adds a "Retired Scheduler Service" sidebar section with two views: Schedules list and Executions log, following the same patterns as Code Tasks. All routes use the `/cron/*` prefix.
...
### retired-scheduler-service (NEW service — `apps/retired-scheduler-service`)
...
### `retired-scheduler-schedules` (owner: retired-scheduler-service)
```typescript
...
### `retired-scheduler-executions` (owner: retired-scheduler-service)
```typescript
interface CronExecution {
  id: string;                        // auto-generated
  scheduleId: string;                // reference to retired-scheduler-schedules
...
- **Port (dev):** 8130 (next after retired-chat-service at 8129)
- **Env var name:** `INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL`
- **Cloud Run service name:** `intexuraos-retired-scheduler-service`
...
### Subtask 1: retired-scheduler-service backend service (`apps/retired-scheduler-service`)
...
**Depends on (contract only, no code dependency):** The API contract defined above. The web app calls the retired-scheduler-service service at `config.retiredSchedulerServiceUrl` (configured via `INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL` env var or dev proxy `/api/retired-scheduler-service`).

**UI Pages:**
1. **Schedules List** (`/#/retired-scheduler-service`) — Table of schedules with status filters, create button, inline pause/resume actions
2. **Schedule Detail** (`/#/retired-scheduler-service/:id`) — View/edit schedule, recent executions for this schedule, manual trigger button
3. **New Schedule** (`/#/retired-scheduler-service/new`) — Form: name, human-language description, services selector (multi-select from available services), instruction textarea for step-based action
4. **Executions Log** (`/#/retired-scheduler-service/executions`) — Filterable log of all executions across schedules, expandable tool call details, agent response, token usage

**Sidebar:** "Retired Scheduler Service" collapsible section between "Code Tasks" and "Research Studio", with subitems: "Schedules" and "Executions"

---

## Subtask 1: retired-scheduler-service Backend Service

### Task 1: Project Scaffolding

**Files:**
- Create: `apps/retired-scheduler-service/package.json`
- Create: `apps/retired-scheduler-service/tsconfig.json`
- Create: `apps/retired-scheduler-service/vitest.config.ts`
- Create: `apps/retired-scheduler-service/Dockerfile`
- Create: `apps/retired-scheduler-service/.dockerignore`
- Create: `apps/retired-scheduler-service/src/index.ts`
- Create: `apps/retired-scheduler-service/src/config.ts`
- Create: `apps/retired-scheduler-service/src/services.ts`
- Create: `apps/retired-scheduler-service/src/server.ts`
- Modify: `firestore-collections.json` — add `retired-scheduler-schedules` and `retired-scheduler-executions`
- Modify: `ecosystem.config.cjs` — add retired-scheduler-service service on port 8130

- [ ] **Step 1: Scaffold package.json**

Copy structure from an existing app (e.g., `apps/bookmarks-agent/package.json`). Set name to `@intexuraos/retired-scheduler-service`. Include dependencies: `fastify`, `@fastify/swagger` (for OpenAPI spec generation), `@intexuraos/common-core`, `@intexuraos/common-http`, `@intexuraos/http-server`, `@intexuraos/http-contracts`, `@intexuraos/infra-firestore`, `@intexuraos/infra-sentry`, `@intexuraos/infra-gemini`, `@intexuraos/llm-contract`, `@intexuraos/llm-prompts`, `cron-parser@^4.9.0` (for cron expression parsing and next-execution calculation — use the `cron-parser` npm package, not alternatives).
...
2. At startup (and periodically), retired-scheduler-service fetches each service's OpenAPI spec
...
Initialize Sentry (`initSentry({ dsn, environment, serviceName: 'retired-scheduler-service' })`), validate env, load config, init services, build server, listen on `parseInt(process.env['PORT'] ?? '8080', 10)`.

- [ ] **Step 7: Update firestore-collections.json**

Add:
```json
"retired-scheduler-schedules": {
  "owner": "retired-scheduler-service",
  "description": "User-defined recurring schedules with cron expressions and action definitions"
},
"retired-scheduler-executions": {
  "owner": "retired-scheduler-service",
...
Add `retired-scheduler-service` entry on port 8130 in Phase 1 (independent). Add `INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL: 'http://localhost:8130'` to `COMMON_SERVICE_URLS`. No `waitForService` needed (retired-scheduler-service is independent, no startup dependencies on other services).

- [ ] **Step 9: Run build to verify scaffolding compiles**

Run: `cd /repo && pnpm install && pnpm --filter @intexuraos/retired-scheduler-service build`

- [ ] **Step 10: Commit**

```bash
git add apps/retired-scheduler-service/ firestore-collections.json ecosystem.config.cjs
git commit -m "feat(retired-scheduler-service): scaffold new service with DI, config, and Firestore collections"
```

### Task 2: Firestore Repositories

**Files:**
- Create: `apps/retired-scheduler-service/src/domain/types.ts`
- Create: `apps/retired-scheduler-service/src/domain/ports/schedule-repository.ts`
- Create: `apps/retired-scheduler-service/src/domain/ports/execution-repository.ts`
- Create: `apps/retired-scheduler-service/src/infra/firestore-schedule-repository.ts`
- Create: `apps/retired-scheduler-service/src/infra/firestore-execution-repository.ts`
- Create: `apps/retired-scheduler-service/src/infra/__tests__/firestore-schedule-repository.test.ts`
- Create: `apps/retired-scheduler-service/src/infra/__tests__/firestore-execution-repository.test.ts`
...
```bash
git commit -m "feat(retired-scheduler-service): add Firestore repositories for schedules and executions"
```

### Task 3: Cron Expression Parsing Use Case

**Files:**
- Create: `apps/retired-scheduler-service/src/domain/use-cases/parse-schedule.ts`
- Create: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/parse-schedule.test.ts`
- Create: `apps/retired-scheduler-service/src/prompts/parse-schedule-prompt.ts`
...
```bash
git commit -m "feat(retired-scheduler-service): add LLM-based schedule parsing use case"
```

### Task 4: Tool Registry — OpenAPI-to-ToolDefinition Generator

**Files:**
- Create: `apps/retired-scheduler-service/src/domain/ports/tool-registry.ts`
- Create: `apps/retired-scheduler-service/src/infra/openapi-tool-registry.ts`
- Create: `apps/retired-scheduler-service/src/infra/__tests__/openapi-tool-registry.test.ts`
...
```bash
git commit -m "feat(retired-scheduler-service): add OpenAPI-based tool registry for dynamic tool generation"
```

### Task 5: Action Executor — LLM Tool-Calling Agent

**Files:**
- Create: `apps/retired-scheduler-service/src/domain/use-cases/execute-action.ts`
- Create: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/execute-action.test.ts`
- Create: `apps/retired-scheduler-service/src/prompts/execute-action-prompt.ts`
...
```bash
git commit -m "feat(retired-scheduler-service): add LLM tool-calling action executor"
```

### Task 6: Tick Handler & Schedule Execution

**Files:**
- Create: `apps/retired-scheduler-service/src/domain/use-cases/handle-tick.ts`
- Create: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/handle-tick.test.ts`
- Create: `apps/retired-scheduler-service/src/domain/use-cases/execute-schedule.ts`
- Create: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/execute-schedule.test.ts`
...
```bash
git commit -m "feat(retired-scheduler-service): add tick handler and schedule execution use cases"
```

### Task 7: Schedule Management Use Case

**Files:**
- Create: `apps/retired-scheduler-service/src/domain/use-cases/manage-schedule.ts`
- Create: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/manage-schedule.test.ts`
...
```bash
git commit -m "feat(retired-scheduler-service): add schedule management use case with CRUD and manual trigger"
```

### Task 8: HTTP Routes

**Files:**
- Create: `apps/retired-scheduler-service/src/routes/schedule-routes.ts`
- Create: `apps/retired-scheduler-service/src/routes/execution-routes.ts`
- Create: `apps/retired-scheduler-service/src/routes/internal-routes.ts`
- Create: `apps/retired-scheduler-service/src/routes/index.ts`
- Create: `apps/retired-scheduler-service/src/routes/__tests__/schedule-routes.test.ts`
- Create: `apps/retired-scheduler-service/src/routes/__tests__/execution-routes.test.ts`
- Create: `apps/retired-scheduler-service/src/routes/__tests__/internal-routes.test.ts`
...
Run: `pnpm --filter @intexuraos/retired-scheduler-service test`

- [ ] **Step 12: Commit**

```bash
git commit -m "feat(retired-scheduler-service): add HTTP routes for schedules, executions, and internal tick"
```

### Task 9: Firestore Composite Index Migrations

**Files:**
- Create: `migrations/062_retired-scheduler-service-composite-indexes.mjs` (root `migrations/` directory — the migration runner only discovers files here, NOT per-app directories)
...
```javascript
// migrations/062_retired-scheduler-service-composite-indexes.mjs
export const indexes = [
  {
    collectionGroup: 'retired-scheduler-schedules',
    fields: [
  ...
  {
    collectionGroup: 'retired-scheduler-executions',
    fields: [
      { fieldPath: 'userId', order: 'ASCENDING' },
      { fieldPath: 'createdAt', order: 'DESCENDING' },
    ],
...
```bash
git commit -m "feat(retired-scheduler-service): add Firestore composite index migrations"
```
...
```hcl
cron_agent = {
  name      = "intexuraos-retired-scheduler-service"
  app_path  = "apps/retired-scheduler-service"
  port      = 8080
  min_scale = 0
  max_scale = 1
}
...
- [ ] **Step 2: Add Cloud Run module for retired-scheduler-service**

Follow existing service module pattern. Include env vars: `INTEXURAOS_GCP_PROJECT_ID`, `INTEXURAOS_INTERNAL_AUTH_TOKEN`, `INTEXURAOS_AUTH_AUDIENCE`, `INTEXURAOS_AUTH_ISSUER`, `INTEXURAOS_AUTH_JWKS_URL`, `INTEXURAOS_SENTRY_DSN`, `INTEXURAOS_ENVIRONMENT`, `INTEXURAOS_GEMINI_APP_API_KEY`, `INTEXURAOS_CODE_AGENT_URL`, and other service URLs as needed. Also add `INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL` to the web app's Terraform env vars for production.

- [ ] **Step 3: Add Cloud Scheduler job**

```hcl
resource "google_cloud_scheduler_job" "cron_agent_tick" {
  name        = "intexuraos-retired-scheduler-service-tick-${var.environment}"
  description = "Trigger retired-scheduler-service tick every minute to evaluate due schedules"
  schedule    = "*/1 * * * *"
  time_zone   = "UTC"
  region      = var.region
...
- [ ] **Step 4: Add IAM binding for Cloud Scheduler → retired-scheduler-service**
...
- [ ] **Step 5: Add retired-scheduler-service to api-docs-hub config**

Update `apps/api-docs-hub/src/config.ts` to include retired-scheduler-service's OpenAPI URL. Add `INTEXURAOS_RETIRED_SCHEDULER_SERVICE_OPENAPI_URL` env var to the api-docs-hub Terraform module and ecosystem.config.cjs.

- [ ] **Step 6: Commit Terraform changes**

```bash
git commit -m "infra(retired-scheduler-service): add Cloud Run service, Cloud Scheduler tick job, and api-docs-hub registration"
```
...
Verify:
- Cloud Run service `intexuraos-retired-scheduler-service` is created
- Cloud Scheduler job `intexuraos-retired-scheduler-service-tick-dev` is created and schedules every minute
- IAM binding allows Cloud Scheduler to invoke the retired-scheduler-service service
...
**Files:**
- Create: `apps/web/src/types/retiredSchedulerService.ts`
- Modify: `apps/web/src/types/index.ts` — re-export new types
- Create: `apps/web/src/services/retiredSchedulerServiceApi.ts`
- Modify: `apps/web/src/config.ts` — add `retiredSchedulerServiceUrl`

- [ ] **Step 1: Add retired scheduler service types**
...
- [ ] **Step 2: Add retiredSchedulerServiceUrl to config.ts**

Add `retiredSchedulerServiceUrl` to the `AppConfig` interface (in `types/index.ts` or wherever it's defined) and `getConfig()`:
```typescript
retiredSchedulerServiceUrl: getServiceUrl('INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL', '/api/retired-scheduler-service'),
```

- [ ] **Step 3: Create retiredSchedulerServiceApi.ts service layer**
...
```bash
git commit -m "feat(web): add retired scheduler service types and API service layer"
```
...
- [ ] **Step 1: Add Retired Scheduler Service sidebar section**

Add between Code Tasks and Research Studio sections:
1. Import `Clock`, `Activity` from `lucide-react`
2. Add `retiredSchedulerServiceItems` array: `[{ to: '/retired-scheduler-service', label: 'Schedules', icon: Clock }, { to: '/retired-scheduler-service/executions', label: 'Executions', icon: Activity }]`
3. Add `isCronAgentOpen` state with localStorage persistence (initialized from `window.location.hash.includes('/retired-scheduler-service')`)
4. Add auto-expand `useEffect` for `/retired-scheduler-service` routes
5. Add collapsible section UI — copy exact Code Tasks pattern with `Clock` icon and "Retired Scheduler Service" label

- [ ] **Step 2: Commit**

```bash
git commit -m "feat(web): add Retired Scheduler Service section to sidebar navigation"
```

### Task 4: Schedules List Page

**Files:**
- Create: `apps/web/src/pages/retired-scheduler-service/CronSchedulesPage.tsx`
- Create: `apps/web/src/pages/retired-scheduler-service/index.ts`
...
- Layout with sidebar
- Page header: "Schedules" title with count + "New Schedule" button (links to `/retired-scheduler-service/new`)
...
- Click row → navigate to `/retired-scheduler-service/:id`
...
```bash
git commit -m "feat(web): add Schedules list page for Retired Scheduler Service"
```

### Task 5: Schedule Detail & Create Pages

**Files:**
- Create: `apps/web/src/pages/retired-scheduler-service/CronScheduleViewPage.tsx`
- Create: `apps/web/src/pages/retired-scheduler-service/CronScheduleNewPage.tsx`
...
- Submit → `createSchedule` → navigate to `/retired-scheduler-service/:id`
...
**Files:**
- Create: `apps/web/src/pages/retired-scheduler-service/CronExecutionsPage.tsx`
...
```bash
git commit -m "feat(web): add Executions log page for Retired Scheduler Service"
```
...
- Modify: `apps/web/vite.config.ts` — add dev proxy for retired-scheduler-service
...
<Route path="/retired-scheduler-service" element={<ProtectedRoute><CronSchedulesPage /></ProtectedRoute>} />
<Route path="/retired-scheduler-service/new" element={<ProtectedRoute><CronScheduleNewPage /></ProtectedRoute>} />
<Route path="/retired-scheduler-service/executions" element={<ProtectedRoute><CronExecutionsPage /></ProtectedRoute>} />
<Route path="/retired-scheduler-service/:id" element={<ProtectedRoute><CronScheduleViewPage /></ProtectedRoute>} />
```
...
```typescript
'/api/retired-scheduler-service': {
  target: 'http://localhost:8130',
  changeOrigin: true,
  rewrite: (path) => path.replace(/^\/api\/retired-scheduler-service/, ''),
},
```

Ensure `INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL` is included in the web app's Vite env config (the `envPrefix` or `define` section) so it's available in production builds.

- [ ] **Step 3: Commit**

```bash
git commit -m "feat(web): add routing and Vite proxy for Retired Scheduler Service pages"
```
