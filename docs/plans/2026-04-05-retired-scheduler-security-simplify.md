# Retired Scheduler Service Security Validation & Config Simplification

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove the redundant `description` field from cron schedules, enforce `allowedOperations` per service to restrict tool access, inject schedule-owner `userId` into all tool calls, and block application-wide endpoints from retired scheduler service access.

**Architecture:** Four independent changes within `apps/retired-scheduler-service` (plus web app frontend). (1) Replace the stored `description` field with a transient `schedule` input for cron parsing. (2) Populate `allowedOperations` on every service in the catalog to create an explicit allowlist. (3) Inject `X-Cron-User-Id` header into all outgoing tool calls so downstream services can verify identity. (4) Audit and exclude application-wide endpoints from the allowlist.
...
**Affected files:**
- `apps/retired-scheduler-service/src/domain/types.ts` — `CronSchedule`, `CreateScheduleInput`, `UpdateScheduleInput`
- `apps/retired-scheduler-service/src/routes/schemas.ts` — request/response JSON schemas
- `apps/retired-scheduler-service/src/routes/schedule-routes.ts` — route handlers and body types
- `apps/retired-scheduler-service/src/domain/use-cases/manage-schedule.ts` — create/update logic
- `apps/retired-scheduler-service/src/domain/use-cases/parse-schedule.ts` — returns `humanSummary` (already does)
- `apps/retired-scheduler-service/src/infra/firestore-schedule-repository.ts` — persistence
- `apps/web/src/types/retiredSchedulerService.ts` — frontend types
- `apps/web/src/services/retiredSchedulerServiceApi.ts` — API client
- `apps/web/src/pages/retired-scheduler-service/CronScheduleNewPage.tsx` — create form
- `apps/web/src/pages/retired-scheduler-service/CronScheduleViewPage.tsx` — view page
- `apps/web/src/pages/retired-scheduler-service/ScheduleListItem.tsx` — list display
- All related test files

### Finding 2: Tool/endpoint registry validation already exists — but `allowedOperations` is never populated

The `ServiceDefinition` type includes an `allowedOperations?: string[]` field (see `apps/retired-scheduler-service/src/config.ts:6`). The `OpenApiToolRegistry` already filters by it (`openapi-tool-registry.ts:129-134`). However, **no service in the catalog sets this field**, meaning ALL `/internal/*` endpoints from every configured service are exposed as tools.
...
**Affected files:**
- `apps/retired-scheduler-service/src/config.ts` — add `allowedOperations` arrays to each catalog entry

### Finding 3: User ID is NOT injected into tool calls — critical security gap

When the retired scheduler service's LLM executes tools, the execution flow is:
...
| File                                                         | Change                                                                                                                                               |
| ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/retired-scheduler-service/src/domain/types.ts`                        | Replace `description` with `scheduleSummary` on `CronSchedule`; replace `description` with `schedule` on `CreateScheduleInput`/`UpdateScheduleInput` |
| `apps/retired-scheduler-service/src/domain/ports/schedule-repository.ts`    | Update `create` signature (input type changes)                                                                                                       |
| `apps/retired-scheduler-service/src/domain/ports/tool-registry.ts`          | Add `ToolExecutionContext` interface; update `getToolsForService`/`getToolsForServices` to accept optional `context?: ToolExecutionContext`          |
| `apps/retired-scheduler-service/src/domain/use-cases/manage-schedule.ts`    | Pass `schedule` (not `description`) to parser; store `scheduleSummary`; pass `userId` to execute deps                                                |
| `apps/retired-scheduler-service/src/domain/use-cases/execute-action.ts`     | Accept `userId`, pass to tool registry and include in prompt                                                                                         |
| `apps/retired-scheduler-service/src/domain/use-cases/execute-schedule.ts`   | Pass `schedule.userId` to `executeAction`                                                                                                            |
| `apps/retired-scheduler-service/src/domain/use-cases/parse-schedule.ts`     | Rename param from `description` to `schedule` (cosmetic)                                                                                             |
| `apps/retired-scheduler-service/src/prompts/execute-action-prompt.ts`       | Add userId context to system prompt (version bump)                                                                                                   |
| `apps/retired-scheduler-service/src/infra/openapi-tool-registry.ts`         | Accept `userId` in `createRunCallback`, inject `X-Cron-User-Id` header                                                                               |
| `apps/retired-scheduler-service/src/infra/firestore-schedule-repository.ts` | Store `scheduleSummary` instead of `description`                                                                                                     |
| `apps/retired-scheduler-service/src/config.ts`                              | Add `allowedOperations` to every catalog entry                                                                                                       |
| `apps/retired-scheduler-service/src/routes/schemas.ts`                      | Update response schema (`scheduleSummary` replaces `description`)                                                                                    |
| `apps/retired-scheduler-service/src/routes/schedule-routes.ts`              | Update body types, pass `schedule` input to manager                                                                                                  |
| `apps/web/src/types/retiredSchedulerService.ts`                            | Replace `description` with `scheduleSummary`; update `CreateScheduleRequest`                                                                         |
| `apps/web/src/services/retiredSchedulerServiceApi.ts`                      | Update request type                                                                                                                                  |
| `apps/web/src/pages/retired-scheduler-service/CronScheduleNewPage.tsx`      | Rename field from "Description" to "Schedule"                                                                                                        |
| `apps/web/src/pages/retired-scheduler-service/CronScheduleViewPage.tsx`     | Display `scheduleSummary` instead of `description`                                                                                                   |
| `apps/web/src/pages/retired-scheduler-service/ScheduleListItem.tsx`         | Display `scheduleSummary` instead of `description`                                                                                                   |
...
- Modify: `apps/retired-scheduler-service/src/domain/ports/schedule-repository.ts`
...
- Modify: `apps/retired-scheduler-service/src/routes/schemas.ts`
- Modify: `apps/retired-scheduler-service/src/routes/schedule-routes.ts`
- Test: `apps/retired-scheduler-service/src/domain/__tests__/types.test.ts`
- Test: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/manage-schedule.test.ts`
- Test: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/parse-schedule.test.ts`
- Test: `apps/retired-scheduler-service/src/routes/__tests__/schedule-routes.test.ts`
- Test: `apps/retired-scheduler-service/src/infra/__tests__/firestore-schedule-repository.test.ts`

- [ ] **Step 1: Update domain types**

In `apps/retired-scheduler-service/src/domain/types.ts`:
...
In `apps/retired-scheduler-service/src/domain/ports/schedule-repository.ts`, the `create` method signature uses `CreateScheduleInput` — it will pick up the type change automatically. No code changes needed, but verify the type flows correctly.

- [ ] **Step 3: Update parse-schedule use case (cosmetic rename)**

In `apps/retired-scheduler-service/src/domain/use-cases/parse-schedule.ts`, rename the parameter from `description` to `schedule`:
...
In `apps/retired-scheduler-service/src/domain/use-cases/manage-schedule.ts`:
...
In `apps/retired-scheduler-service/src/infra/firestore-schedule-repository.ts`, the `create` method builds a `CronSchedule` object. Update:
...
In `apps/retired-scheduler-service/src/routes/schemas.ts`, update `scheduleResponseSchema`:
```typescript
// Replace `description: { type: 'string' }` with:
scheduleSummary: { type: 'string' },
```

In `apps/retired-scheduler-service/src/routes/schedule-routes.ts`:
...
```bash
git add apps/retired-scheduler-service/src/
git commit -m "feat(retired-scheduler-service): replace description with schedule input and scheduleSummary storage
...
- Test: `apps/retired-scheduler-service/src/__tests__/config.test.ts`

This task requires auditing each service's `/internal/*` endpoints and deciding which operationIds are safe for retired scheduler service use. The `allowedOperations` mechanism already exists and is tested — we just need to populate the arrays.

- [ ] **Step 1: Audit and populate allowedOperations**

In `apps/retired-scheduler-service/src/config.ts`, add `allowedOperations` to every entry in `INTERNAL_API_SERVICE_CATALOG`. Below is the allowlist based on the security audit:
...
const INTERNAL_API_SERVICE_CATALOG = [
  ...
  {
    key: 'retired-checklist-service',
    name: 'Retired Checklist Service',
    baseUrlEnvVar: 'INTEXURAOS_RETIRED_CHECKLIST_SERVICE_URL',
    openApiUrlEnvVar: 'INTEXURAOS_RETIRED_CHECKLIST_SERVICE_OPENAPI_URL',
  ...
  {
    key: 'retired-chat-service',
    name: 'Retired Chat Service',
    baseUrlEnvVar: 'INTEXURAOS_RETIRED_CHAT_SERVICE_URL',
    openApiUrlEnvVar: 'INTEXURAOS_RETIRED_CHAT_SERVICE_OPENAPI_URL',
    allowedOperations: [],  // REVIEW: needs operationId audit of retired-chat-service internal routes
  },
  {
    key: 'code-agent',
    name: 'Code Agent',
  ...
  {
    key: 'retired-scheduler-service',
    name: 'Retired Scheduler Service',
    baseUrlEnvVar: 'INTEXURAOS_RETIRED_SCHEDULER_SERVICE_URL',
    openApiUrlEnvVar: 'INTEXURAOS_RETIRED_SCHEDULER_SERVICE_OPENAPI_URL',
    allowedOperations: [],  // BLOCKED: retired-scheduler-service should not call itself
  },
  {
...
```bash
git add apps/retired-scheduler-service/src/config.ts
git commit -m "feat(retired-scheduler-service): populate allowedOperations for all services in catalog
...
app-settings-service, commands-agent, retired-scheduler-service) get empty arrays, blocking
...
- Modify: `apps/retired-scheduler-service/src/domain/use-cases/execute-schedule.ts`
- Modify: `apps/retired-scheduler-service/src/infra/openapi-tool-registry.ts`
- Modify: `apps/retired-scheduler-service/src/domain/ports/tool-registry.ts`
- Modify: `apps/retired-scheduler-service/src/prompts/execute-action-prompt.ts`
- Test: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/execute-action.test.ts`
- Test: `apps/retired-scheduler-service/src/domain/use-cases/__tests__/execute-schedule.test.ts`
- Test: `apps/retired-scheduler-service/src/infra/__tests__/openapi-tool-registry.test.ts`

- [ ] **Step 1: Thread userId through the execution chain**

In `apps/retired-scheduler-service/src/domain/use-cases/execute-action.ts`, add `userId` parameter:
...
In `apps/retired-scheduler-service/src/domain/use-cases/execute-schedule.ts`, pass `schedule.userId`:
...
In `apps/retired-scheduler-service/src/domain/ports/tool-registry.ts`, add optional context to tool retrieval:
...
In `apps/retired-scheduler-service/src/infra/openapi-tool-registry.ts`, thread `userId` through the tool generation pipeline so it's captured at tool-creation time rather than grafted onto cached tools:
...
In `apps/retired-scheduler-service/src/domain/use-cases/execute-action.ts`:
...
In `apps/retired-scheduler-service/src/prompts/execute-action-prompt.ts`, add userId to the prompt input and system prompt. Bump version to `3.0.0` (behavior change):
...
```bash
git add apps/retired-scheduler-service/src/
git commit -m "feat(retired-scheduler-service): inject schedule-owner userId into all tool calls
...
- Modify: `apps/web/src/services/retiredSchedulerServiceApi.ts`
...
- Modify: `apps/web/src/pages/retired-scheduler-service/ScheduleListItem.tsx`
- Test: `apps/web/src/__tests__/CronScheduleNewPage.test.tsx`

- [ ] **Step 1: Update frontend types**

In `apps/web/src/types/retiredSchedulerService.ts`:
...
In `apps/web/src/services/retiredSchedulerServiceApi.ts`, the `createSchedule` function sends `CreateScheduleRequest` — the type change propagates automatically. Also update the `updateSchedule` function's `Partial<CreateScheduleRequest ...>` type.

- [ ] **Step 3: Update CronScheduleNewPage**

In `apps/web/src/pages/retired-scheduler-service/CronScheduleNewPage.tsx`:
...
In `apps/web/src/pages/retired-scheduler-service/CronScheduleViewPage.tsx`:
...
In `apps/web/src/pages/retired-scheduler-service/ScheduleListItem.tsx`:
...
3. **Audit linear-agent `x-user-id` header** — The linear-agent internal issues routes accept `x-user-id` as a header without validation. This is a vulnerability independent of the retired scheduler service.
