...
- 100% branch coverage required on new code; use `v8 ignore` only with valid categories (`schema`, `async-timing`, `test-infra`).
...
- `apps/web/src/components/intex-agent/InstructionRow.tsx`
- `apps/web/src/components/intex-agent/InstructionEditor.tsx`
- `apps/web/src/components/__tests__/intex-agent/InstructionRow.test.tsx`
- `apps/web/src/components/__tests__/intex-agent/InstructionEditor.test.tsx`
...
- `apps/web/src/components/sidebar/useSidebarState.ts` — add `isIntexAgentOpen` state, hydrate from URL hash.
...
- [ ] **Step 1.1:** Write the failing types + port tests. Test compile-only, but include `expectInstructionRepositoryContract` to assert the port's four methods exist with the right signatures.
...
- [ ] **Step 3.1:** Write failing tests using a fake Firestore (mirror `FirestoreSessionRepository` test style — in-memory map keyed by `${userId}/${instructionId}`).
  ...
  - `delete` is idempotent — second call does not throw.
- [ ] **Step 3.2:** Run test — fail.
- [ ] **Step 3.3:** Implement `FirestoreInstructionRepository` with collection constant `INTEX_AGENT_USER_INSTRUCTIONS_COLLECTION = 'intex_agent_user_instructions'`, document id `${userId}_${randomUUID()}` (consistent with other collections that do not embed userId in id — instead store `userId` in the doc and filter via `where('userId', '==', userId)`).
- [ ] **Step 3.4:** Tests pass.
...
- [ ] **Step 4.1:** Write failing tests using `app.inject()` (mirror `sessionRoutes.test.ts`).
  ...
  - `DELETE /user-instructions/:id` returns 204.
...
- [ ] **Step 7.3:** Re-export from `services/index.ts`.
...
- [ ] **Step 8.2:** Implement hook using `useApiClient` (NOT direct `getAccessToken`). Use optimistic updates with revert on failure (mirror `WhatsAppPreferencesCard`).
- [ ] **Step 8.3:** Tests pass.
...
- [ ] **Step 9.4:** Run `pnpm --filter web exec vitest run src/components/sidebar` — pass.
- [ ] **Step 9.5:** Commit: `feat(web): INTEX Agent sidebar section`.
...
**Files:**
- Create: `apps/web/src/components/intex-agent/InstructionRow.tsx`
- Create: `apps/web/src/components/intex-agent/InstructionEditor.tsx`
- Create: `apps/web/src/pages/IntexAgentConfigurationPage.tsx`
- Tests: `apps/web/src/components/__tests__/intex-agent/InstructionRow.test.tsx`, `InstructionEditor.test.tsx`, `apps/web/src/pages/__tests__/IntexAgentConfigurationPage.test.tsx`

- [ ] **Step 10.1:** Write failing test for `InstructionRow`: renders `text`, calls `onEdit` / `onDelete` on click.
- [ ] **Step 10.2:** Implement `InstructionRow` with `Pencil` / `Trash2` icons, dark-mode aware Tailwind classes consistent with `WhatsAppPreferencesCard`.
- [ ] **Step 10.3:** Write failing test for `InstructionEditor`: textarea + Save (disabled while empty / saving) + Cancel; calls `onSave(text)`.
- [ ] **Step 10.4:** Implement `InstructionEditor` using `Button` / `Input` / `Card` from `@/components`.
- [ ] **Step 10.5:** Write failing page tests:
...
- [ ] **Step 10.6:** Implement page using `useIntexAgentPreferences`. Include header `INTEX Agent Configuration` and helper text describing the example from the spec verbatim: "These instructions are injected as a User Preferences block whenever the agent executes a prompt. Example: 'When I add a calendar event with Monika, also invite monikamaupa@gmail.com'."
- [ ] **Step 10.7:** Tests pass.
- [ ] **Step 10.8:** Commit: `feat(web): INTEX Agent configuration page`.
...
| DELETE | `/user-instructions/:id`             | Delete. Idempotent; 204 on success, 404 if not owned.                       |
