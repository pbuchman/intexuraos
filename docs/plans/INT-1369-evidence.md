...
**Architecture:** This work splits cleanly into two tracks. The remaining codebase reuses `packages/internal-clients/src/user-service/client.ts#getLlmClient()` as the single source of truth for default-model plus fallback resolution, then removes service-local Gemini assumptions in hellscript-agent, image-service, linear-agent, and retired-chat-service. The orchestrator remains platform-owned, but stops hardcoding one verifier model per component by parsing a single ordered environment variable and constructing primary/secondary validation clients that both completion verification and `agent-compliance-validator` can share.
...
| Chat-agent migration          | `apps/retired-chat-service/src/services.ts`, `apps/retired-chat-service/src/routes/chatRoutes.ts`, related tests                                                                                                                                       | Keep guest Gemini fixed, but route authenticated user generation through the default user client with fallback       |
...
- Modify tests in `apps/retired-chat-service/src/__tests__/`
...
| 8   | retired-chat-service       | `src/services.ts`                          | keep guest Gemini, migrate authenticated path        |
...
| `apps/retired-scheduler-service/src/index.ts`                                                                                                                                                                    | `Gemini25Flash` tool-calling + generate clients | **No change in this task** | platform-owned scheduler, no user context                                                               |
...
4. Finish with retired-chat-service, keeping the guest split explicit and covered by tests.
