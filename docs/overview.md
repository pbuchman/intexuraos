...
An AI-native personal operating system — 24 specialized agents that turn your voice notes, messages, and ideas into research, code, calendar events, and organized work, all from WhatsApp and a single web dashboard.
...
**[WhatsApp Service](services/whatsapp-service/features.md)** is your mobile command center. Send a text message, record a voice note, or share a link — and the system routes it to the right agent automatically. Voice transcription understands your specialized vocabulary — 90 domain-specific terms, project names, and technical words — without garbling them. When an agent needs your decision, an interactive approval button arrives in the same conversation thread. Four interactions with four different agents, none requiring you to leave WhatsApp.

**[Web App](services/web/features.md)** is where you observe, approve, and control everything the agents are doing on your behalf. Watch code being written in real time with color-coded log streams. Approve action items and code task designs from a single inbox. Review research reports, manage your calendar, track your Linear board, and configure every connected service — all in one interface you can install on your phone's home screen like a native app. Share content from any app on your phone directly into IntexuraOS through the system share menu.
...
**[Calendar Agent](services/calendar-agent/features.md)** puts events on your Google Calendar from voice notes and text messages. Say "Dentist next Tuesday at 3pm" and a preview appears with the title, date, time, duration, and the AI's reasoning. Approve it, and the event is created. Relative dates, all-day events, locations, and multilingual input — including Polish — all work without a settings page. Vague messages that lack enough detail are saved for review instead of discarded.
...
**[Linear Agent](services/linear-agent/features.md)** turns voice notes and quick messages into structured, prioritized issues in Linear (a project tracking tool). The AI extracts a title, separates functional requirements from technical details, and reads urgency cues from your words — "ASAP" maps to Urgent, "when you have time" maps to Low. Your board loads instantly in the web dashboard from a local copy updated in real time, so you never wait for Linear's servers. The code agent updates your board as it works, closing the loop from thought to finished code to tracked progress.
...
**[Code Agent](services/code-agent/features.md)** is the interface you interact with. Submit a task by WhatsApp voice note, text message, or the web dashboard. The agent creates a Linear issue, produces a design document explaining its approach, and stops. You review the design on your own schedule. Approve it, and the agent writes tests, writes code, runs the test suite, and opens a code change for review. Leave a comment on GitHub, and the agent picks up your feedback with full context. Per-user limits on concurrency, hourly rate, and daily spend keep costs predictable — the estimated cost per task is about $1.17.

**[Orchestrator](services/orchestrator/features.md)** runs on your own hardware. Your source code never leaves your network. Any Unix machine becomes a worker station, connected to the platform through a single secure outbound connection — no firewall changes, no open ports. Each task gets its own branch, its own container, its own credentials — two concurrent tasks never see each other's files. An independent verifier confirms completion by checklist and AI review, not worker self-reporting. A sensitive file guard scans every commit for passwords and secrets, reverting anything suspicious before it reaches your repository.
...
When the work involves code, execution happens on your own infrastructure. The orchestrator receives the task, creates an isolated container with a fresh copy of your repository, and the Claude worker writes the code, runs the tests, and produces a finished change ready for your review. An independent verifier confirms the result. Logs stream back to your web dashboard in real time. The machine that runs all of this starts and stops on a schedule, so you pay for compute only when you are using it.
