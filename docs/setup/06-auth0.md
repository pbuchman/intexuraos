...
## 15. Social Login: Include Profile Claims in Tokens

When using social login (Google, GitHub, etc.), user profile information like `name` and `email` must be explicitly mapped to the JWT tokens. Without this configuration, these claims will be missing from tokens even though the social provider returns them.

### Why This Matters

IntexuraOS uses `name` and `email` claims for:

- **Research Attribution**: "Generated by" author info in research documents
- **User Display**: Showing user names in the UI
- **Notifications**: Addressing users by name in messages

### Configure Auth0 Action for Profile Claims

1. Go to **Actions** → **Library** → **Build Custom**
2. Create a new action:
   - **Name**: `Add Profile Claims to Tokens`
...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // Add to ID token as well (for web clients)
  if (event.user.name) {
...
6. Drag your new action into the flow (after Login but before Complete)
7. Click **Apply**

### Verify Claims Are Present

After configuring, test by authenticating and decoding the access token:
...
```json
{
  ...
  "https://intexuraos.cloud/name": "John Doe",
  "https://intexuraos.cloud/email": "john@example.com",
  ...
}
```

### Troubleshooting Missing Claims

**Symptom:** Research documents show no "Generated by" attribution

**Causes:**

- Auth0 Action not deployed or not in flow
- Social connection not returning profile data
- Missing `profile` and `email` scopes

**Fix:**

1. Verify Action is deployed and in Login flow
2. Check social connection settings include profile/email scopes
3. Ensure `profile email` scopes are requested during authentication

### Google-Specific Configuration

For Google social login specifically:

1. Go to **Authentication** → **Social** → **google-oauth2**
2. Click **Settings**
3. Under **Permissions**, ensure these scopes are enabled:
   - `email`
   - `profile`
4. Click **Save**

> **Note**: Google always returns basic profile info, but Auth0 Actions are needed to map these to JWT claims.

## 16. References (2026-01-30)
