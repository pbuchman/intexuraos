...
7. Keep Matrix tokens, WhatsApp bridge state, `.env`, and Google credential JSON out of Git.
...
Daily calendar notifications depend on this outbound setup on the Matrix host. Backend schedule configuration can be enabled before the Matrix host is ready, so delivery remains blocked until the host has a valid Matrix access token, outbound auth token file, and outbound targets mapping file for the user source account.
