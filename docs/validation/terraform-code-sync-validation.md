...
| `docker/code-worker`    | Worker      | CRITICAL | No Cloud Run or Cloud Function module. Worker containers are launched dynamically by the orchestrator (not via Terraform-managed infrastructure). By design — these are ephemeral containers. |
