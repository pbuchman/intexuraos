...
**Files:** `processCodeAction.ts:226`, `codeRoutes.ts:1226`
...
**File:** `apps/code-agent/src/domain/usecases/processCodeAction.ts:226`

```typescript
systemPromptHash: 'system-prompt-hash-v1', // TODO: Compute from actual system prompt
```

**File:** `apps/code-agent/src/routes/codeRoutes.ts:1226`
...
### 1. codeRoutes.ts is ~3600 lines (SRP violation)
...
**Impact:** DRY violation. Any change to the secret format must be applied in four places.

**Remediation:** Extract to a shared utility in `domain/utils/secrets.ts` and import in all use cases.
...
**File:** `apps/code-agent/src/routes/codeRoutes.ts:29`
