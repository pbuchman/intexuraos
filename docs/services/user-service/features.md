...
4. You select "Claude Haiku 3.5" as your default fast model. The service verifies you have an Anthropic API key configured before accepting — you cannot select a model for a provider you have not set up. Every agent across the platform now uses it for quick generation tasks.
5. You connect your Google account for calendar access. One OAuth flow, and the calendar-agent can read your schedule indefinitely — tokens refresh automatically in the background.
6. You connect your GitHub account for code automation. Another OAuth flow, and the code-agent can create pull requests on your behalf. GitHub tokens do not expire unless revoked, so there is no refresh logic.
7. You set Speechmatics as your preferred transcription provider. Voice notes from WhatsApp are now processed through your chosen engine.
8. You set your timezone to Europe/Berlin. All time-aware features across the platform now display and process dates in your local timezone.

Eight settings, configured once. From this point on, you never think about credentials again.
