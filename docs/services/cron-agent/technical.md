...
graph TB
    ...
    Action -->|report token usage| UsageSvc
    Manage --> Firestore
    Execute --> Firestore
    ...
    class Client,Scheduler,Gemini,TargetSvcs,UsageSvc external
```
...
sequenceDiagram
    ...
    participant Usage as LLM Usage Service

    Note over Scheduler,Usage: Schedule Creation
    CronAgent->>Gemini: Parse "every weekday at 9am"
    Gemini-->>CronAgent: {"cronExpression": "0 9 * * 1-5", "humanSummary": "..."}
    CronAgent->>Usage: Report parse token usage
    CronAgent->>Firestore: Create cron_schedules doc

    Note over Scheduler,Usage: Scheduled Execution
    ...
    CronAgent->>Usage: Report execution token usage
...
| ----------- | ------------------------------------------------------------------------- | ---------- |
| `6f2a13c14` | Remove pricing from client configs                                        | 2026-04-15 |
| `b6ae0d97c` | Remove Data Insights references from shared services, catalogs, and tests | 2026-04-14 |

**LLM Usage Tracking (INT-1342, INT-1392)**

| Commit      | Description                                                       | Date       |
| ----------- | ----------------------------------------------------------------- | ---------- |
| `8b1211dc0` | Wire `HttpInternalAuthUsageSink` in all LLM callers               | 2026-04-12 |
| `8aae64e4b` | Make `promptType` required in `LlmGenerateClient` and all callers | 2026-04-17 |

These are cross-cutting infrastructure changes — no cron-agent-specific features were added in this release. The service now reports LLM token usage to the centralized `llm-usage-service` via `HttpInternalAuthUsageSink` instead of handling pricing locally.
...
| `status`           | `ScheduleStatus` | `'active'`, `'paused'`, or `'deleted'`         |
...
| `userId`        | `string`          | Owner user ID                                         |
| `status`        | `ExecutionStatus` | `'running'`, `'success'`, `'failure'`, or `'skipped'` |
| `trigger`       | `TriggerType`     | `'scheduled'` or `'manual'`                           |
...
The cron-agent maintains an internal catalog of target services with per-service operation allowlists. Only operations explicitly listed in `allowedOperations` are exposed as tools. Services with empty allowlists expose all their `/internal/*` endpoints without restriction.
...
| `INTEXURAOS_LLM_USAGE_SERVICE_URL` | URL for centralized LLM usage tracking  | Yes      |
