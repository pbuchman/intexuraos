...
**Analysis Run:** v3.3.0 documentation refresh
...
**Note:** The combined `Crawl4AIClient` class that used Crawl4AI's built-in LLM extraction is superseded by the separated architecture (PageContentFetcher for crawling, LlmSummarizer for user's LLM). The old client remains for reference but is not used in production paths. The convenience functions are deprecated in favour of the PromptBuilder pattern.
