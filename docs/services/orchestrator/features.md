...
This is the engineering decision at the heart of the system: the model that writes the code must never be the model that judges the result. Claude or Codex executes the task. A configurable chain of validation models evaluates whether the task was actually completed. An independent compliance validator audits the full session transcript. A deterministic enforcement layer handles what no model can be trusted to do consistently. Multiple independent trust boundaries, each checking the one before it.

The Completion Verifier sends the agent's final output to a prioritized list of validation models — configurable via `INTEXURAOS_ORCHESTRATOR_VALIDATION_MODELS` with support for both Gemini and OpenRouter models. The verifier tries each model in order, falling back to the next if one fails. It extracts structured metadata and validates it against agent-specific schemas — one for planning, one for execution, one for pull request work, one for code review, one for remediation, one for ask-agent. This is not a "looks good" check. It is structured data extraction with schema enforcement. If mandatory fields are missing or the contract is unmet, the task is automatically retried with a targeted prompt that names exactly which criteria failed. Fatal crashes — out-of-memory kills and segfaults — skip the validation call entirely and trigger an immediate retry, because there is no output to evaluate. All execution outcomes — including tasks where the agent determines the work was already completed — require an evidence PR URL.
...
The Review Agent supports five review scopes — `code_quality`, `security`, `architecture`, `plan_review`, and `test_quality` — each targeting a specific dimension of the pull request. The `test_quality` scope provides comprehensive analysis of test files: coverage gaps, assertion quality, edge case handling, test naming, and anti-patterns.
...
- **Thirteen worker type presets** — Anthropic (auto, opus, sonnet), MiniMax, Xiaomi MiMo (mimo-pro), Alibaba Cloud DashScope (GLM, Qwen, Kimi), OpenRouter free tier, and Codex (standard and xhigh) — all through a single orchestrator
...
- **Validation model dependency** — Completion verification requires at least one reachable validation model (Gemini or OpenRouter); if all models in the chain are unreachable during a task, that task fails rather than allowing unverified results through
