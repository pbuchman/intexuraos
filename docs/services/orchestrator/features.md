...
When a task arrives, the orchestrator creates a dedicated copy of the repository on its own branch. It then spawns a Docker container locked to that workspace — each container runs with restricted capabilities and cannot reach outside its own workspace or modify the host machine. Two tasks running simultaneously never see each other's files, credentials, or output. The default capacity is two concurrent tasks, configurable based on your hardware. A Docker health gate checks daemon availability and disk health before accepting new tasks, and container creation times out after two minutes to prevent hung dispatches.
...
Autonomous agents have a persistent habit: declaring the work done when it is not. The orchestrator addresses this with a two-stage independent verification pipeline after every attempt.

**Stage 1 — Completion Verification.** A Gemini model evaluates the semantic completeness of each agent's final output block from Claude responses, extracting structured metadata (PR URLs, skill usage proofs, outcome labels) and validating them against agent-specific Zod schemas. The worker's own assessment of its performance is not consulted. If verification fails and the attempt count has not reached the configurable maximum (three by default), the orchestrator automatically launches a follow-up attempt with a resume prompt listing exactly which criteria were not met. Fatal exit codes (137/139 — OOM kill and segfault) skip the Gemini call entirely and trigger an immediate retry.
...
### Split Planning, Execution, Review, and PR Follow-Up
...
The orchestrator persists task state to disk. If the machine reboots, the process crashes, or Docker restarts, it discovers running containers on startup and attempts to adopt them — re-attaching to live containers so tasks continue without interruption. If a container has exited or is unreachable, the platform is notified and the container is cleaned up. Stale workspaces and leftover containers are garbage-collected automatically through periodic cleanup. Each attempt has a two-hour timeout — with a warning at one hour and fifty-five minutes and a hard kill at two hours — so a stuck task never occupies a worker indefinitely. The repository manager validates and sanitizes the local clone on every startup, handling credential leaks in remote URLs and gracefully degrading when fetch operations fail.
...
- **Self-correcting execution** — Failed verification triggers automatic follow-up attempts with targeted resume prompts; fatal exit codes (OOM, segfault) trigger immediate retries
...
- **Four-agent workflow** — Planning, Execution, Review, and Pull Request agents each have specialized contracts, enabling a structured pipeline from issue to merged code
