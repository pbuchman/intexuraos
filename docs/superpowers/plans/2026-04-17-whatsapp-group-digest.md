...
## Phase 1 /simplify Deltas (applied after task 1.13)

The `/simplify` skill review surfaced several issues that were fixed before opening the Phase 1 PR. Subsequent phases should be aware of these to avoid regressions.

| Item                                                                   | Original (in plan above)                                                                                                                         | After /simplify                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DigestError` variants in Phase 1                                      | 6 (input-invalid, llm-call-failed, zod-validation-failed, repair-exhausted, lock-held, persistence-failed) + 6 factories                         | **4** variants only. `lock-held` and `persistence-failed` removed — they belong with the code that emits them. **Phase 2 must extend the union** in `digestErrors.ts` when it adds `runDigestForGroup` (lockHeld) and the repository-error wrapping (persistenceFailed). The factory functions and union members were removed from Task 1.5; re-add them in Task 2.7 alongside their first call site. |
| `aggregateDigest` repair loop                                          | Single `for` loop with `let prompt`, `let promptType`, `let lastResponseContent`, `let lastErrorMessage` re-assigned across iterations           | Refactored: explicit "initial call then repair loop" structure via a `callAndParse` helper returning a discriminated union (`'ok' \                                                                                                                                                                                                                                                                   | 'llm-error' \ | 'invalid'`). Eliminates redundant initial-state mutation. Public exports unchanged. |
| `aggregateDigest` Zod error formatting                                 | `validation.error.message` (raw, unbounded)                                                                                                      | `formatZodErrors(validation.error)` from `@intexuraos/llm-utils` (capped at 5 issues, friendlier for the LLM repair prompt). New dependency on `@intexuraos/llm-utils` added to `apps/mobile-notifications-service/package.json`.                                                                                                                                                                     |
| `PROMPT_TYPE_AGGREGATE` / `PROMPT_TYPE_REPAIR` / `MAX_REPAIR_ATTEMPTS` | File-private `const`s                                                                                                                            | **Exported.** Tests import them; raw string literals removed from test assertions.                                                                                                                                                                                                                                                                                                                    |
| `digestPrompt.ts` example serialization                                | `JSON.stringify(COLD_START_EXAMPLE, null, 2)` inline in the template literal — runs every `buildDigestPrompt()` call                             | Hoisted to module-level `COLD_START_JSON` / `WITH_CONTEXT_JSON` constants — serialized once at module load.                                                                                                                                                                                                                                                                                           |
| `digestRepairPrompt.ts` schema reference                               | `Schema docelowa: ... (definicje w packages/llm-prompts/src/digest/digestPrompt.ts oraz w packages/mobile-notifications-service domain/schemas)` | Generic phrasing; no repository-internal file paths leaked into the LLM prompt.                                                                                                                                                                                                                                                                                                                       |
| `// Prompt version: 1.0.0` trailing comment                            | Present on both `digestPrompt.ts` and `digestRepairPrompt.ts`                                                                                    | Removed. The exported `DIGEST_PROMPT_VERSION` / `DIGEST_REPAIR_PROMPT_VERSION` constants are the single source of truth.                                                                                                                                                                                                                                                                              |
| `examples.ts` header                                                   | Multi-paragraph note referencing `INT-1382` and a specific share URL                                                                             | Trimmed to a single sentence about the schema contract. No issue IDs or ephemeral URLs.                                                                                                                                                                                                                                                                                                               |
