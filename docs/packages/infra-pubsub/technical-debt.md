...
`WhatsAppSendPublisherConfig`, `TodosProcessingPublisherConfig`, and `CalendarPreviewPublisherConfig` are identical (`{ projectId, topicName, logger }`). Each is defined separately in `types.ts`.
